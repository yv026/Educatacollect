...
with st.form("form_collecte", clear_on_submit=True):
    ...
    if submitted:
        if not nom.strip():
            ...
            st.success(f"✅ **{nom.strip()}** enregistré(e) ! Total : **{len(st.session_state.data)}** étudiants.")
            st.balloons()

st.markdown('<div class="divider"></div>', unsafe_allow_html=True)
st.markdown("**📂 Importer un fichier CSV**")
up = st.file_uploader("Choisissez un fichier CSV", type="csv")
if up:
    try:
        tmp = pd.read_csv(up)
        st.session_state.data = pd.concat([st.session_state.data, tmp], ignore_index=True)
        save_data(st.session_state.data)
        st.success(f"✅ {len(tmp)} lignes importées avec succès !")
...
if df.empty:
    ...
    with col1:
        ...
        fig = px.pie(gc, names="Genre", values="N", title="Répartition par genre",
    ...
    with col2:
        ...
        fig2 = px.bar(nc, x="Niveau", y="N", title="Par niveau",
    ...
    with col3:
        ...
        fig3.update_layout(title="Notes par matière", paper_bgcolor="white")
        st.plotly_chart(fig3, use_container_width=True)
    with col4:
        rc = df["Région"].value_counts().reset_index(); rc.columns = ["Région","N"]
        fig4 = px.bar(rc, x="N", y="Région", orientation="h", title="Par région",
...
if df.empty:
    ...
    with tab1:
        ...
        fig = px.histogram(df, x="Moyenne", nbins=15,
                           color_discrete_sequence=["#3b82f6"],
    ...
    with tab2:
        ...
        fig3 = px.scatter(df, x="Heures_étude_jour", y="Moyenne",
                          color="Filière", size="Satisfaction",
                          hover_data=["Nom","Niveau"], trendline="ols",
                          title="Heures d'étude ↔ Moyenne")
    ...
    with tab3:
        ...
        fig4 = px.bar(by_fil, x="Filière", y="Moyenne", title="Moyenne par filière",
                      color_discrete_sequence=["#3b82f6"])
        fig4.add_hline(y=10, line_dash="dash", line_color="#ef4444")
        fig4.update_layout(paper_bgcolor="white"); st.plotly_chart(fig4, use_container_width=True)
        by_niv = df.groupby("Niveau")["Moyenne"].mean().reset_index()
        fig5 = px.bar(by_niv, x="Moyenne", y="Niveau", orientation="h",
                      title="Moyenne par niveau", color_discrete_sequence=["#10b981"])
        fig5.update_layout(paper_bgcolor="white"); st.plotly_chart(fig5, use_container_width=True)
```
...
if df.empty:
    ...
    st.markdown(f"**{len(df)} enregistrement(s)** dans la base.")
    ...
    col_a.download_button("⬇️ Télécharger CSV", data=df.to_csv(index=False).encode("utf-8"),
                          file_name="EduData.csv", mime="text/csv")
    buf = io.BytesIO(); df.to_excel(buf, index=False, engine="openpyxl")
    col_b.download_button("⬇️ Télécharger Excel", data=buf.getvalue(),
